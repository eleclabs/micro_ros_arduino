...
// Motor control pins
int INA1 = 22; int INB1 = 23;  int pwma =  ;
int INA2 = 19; int INB2 = 21;
int INA3 = 4;  int INB3 = 27;
int INA4 = 17; int INB4 = 16;
...
void move(int rightspeed, int leftspeed) {
  ...
  if (leftspeed > 0) {
    ...
    analogWrite(INA2, 0);
    analogWrite(INB2, leftPWM);
  } else if (leftspeed < 0) {
    analogWrite(INA1, 0);
    analogWrite(INB1, leftPWM);
    analogWrite(INA2, leftPWM);
    analogWrite(INB2, 0);
  } else {
    analogWrite(INA1, 0); analogWrite(INB1, 0);
    analogWrite(INA2, 0); analogWrite(INB2, 0);
  }
  
  // RIGHT MOTOR
  if (rightspeed > 0) {
    analogWrite(INA4, 0);
    analogWrite(INB4, rightPWM);
    analogWrite(INA3, 0);
    analogWrite(INB3, rightPWM);
  } else if (rightspeed < 0) {
    analogWrite(INA4, 0);
    analogWrite(INB4, rightPWM);
    analogWrite(INA3, rightPWM);
    analogWrite(INB3, 0);
  } else {
    analogWrite(INA3, 0); analogWrite(INB3, 0);
    analogWrite(INA4, 0); analogWrite(INB4, 0);
  }
}
...
void setup() {
  ...
  set_microros_wifi_transports("EMB5325", "cdti12345", "192.168.1.100", 8888);  //IP ของเครื่อง Host PC

  setupEncoders();

  pinMode(INA1, OUTPUT); pinMode(INB1, OUTPUT);
  pinMode(INA2, OUTPUT); pinMode(INB2, OUTPUT);
  pinMode(INA3, OUTPUT); pinMode(INB3, OUTPUT);
  pinMode(INA4, OUTPUT); pinMode(INB4, OUTPUT);
